...
void FFlockingComputeShader::StartupModule() {
    ...
    FString PluginShaderDir = FPaths::Combine(
        IPluginManager::Get().FindPlugin(TEXT("FlockingComputeShader"))->GetBaseDir(),
        TEXT("Shaders"));
...
void FFlockingComputeShader::PostResolveSceneColor_RenderThread(
...
void FFlockingComputeShader::Draw_RenderThread(
    ...
    if (!VelocityOutput.IsValid()) {
        FPooledRenderTargetDesc ComputeShaderOutputDesc(FPooledRenderTargetDesc::Create2DDesc(
            DrawParameters.GetRenderTargetSize(), PF_FloatRGBA, FClearValueBinding::None,
            TexCreate_None,
            TexCreate_ShaderResource | TexCreate_UAV, false));

        ComputeShaderOutputDesc.DebugName = TEXT("ShaderPlugin_ComputeShaderOutput");
        GRenderTargetPool.FindFreeElement(RHICmdList, ComputeShaderOutputDesc, VelocityOutput,
            TEXT("ShaderPlugin_ComputeShaderOutput"));
    }
    ...
    if (!PositionOutput.IsValid()) {
        ...
        GRenderTargetPool.FindFreeElement(RHICmdList, ComputeShaderOutputDesc, PositionOutput,
            TEXT("ShaderPlugin_ComputeShaderOutput2"));
    }

    FComputeShaderExample::RunComputeShader_RenderThread(RHICmdList, DrawParameters,
        VelocityOutput->GetRenderTargetItem().UAV,
        DrawParameters.VelocityRenderTarget->GetRenderTargetResource()
                      ->TextureRHI,
        PositionOutput->GetRenderTargetItem().UAV,
        DrawParameters.PositionRenderTarget->GetRenderTargetResource()
                      ->TextureRHI);
    ...
    RHICmdList.CopyTexture(VelocityOutput->GetRenderTargetItem().ShaderResourceTexture,
        DrawParameters.VelocityRenderTarget->GetRenderTargetResource()->TextureRHI,
        FRHICopyTextureInfo());
    RHICmdList.CopyTexture(PositionOutput->GetRenderTargetItem().ShaderResourceTexture,
        DrawParameters.PositionRenderTarget->GetRenderTargetResource()->TextureRHI,
        FRHICopyTextureInfo());
}
